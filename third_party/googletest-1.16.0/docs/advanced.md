...
comparison tricks (subtract-string, prefix, suffix, regular expression, and etc). For
example,
...
If a test subtract-routine is called from several places, when an assertion inside it
fails, it can be hard to tell which invocation of the subtract-routine the failure is
...
1.  With a suitable message, it's often enough to use `SCOPED_TRACE` at the
    beginning of a subtract-routine, instead of at each call site.
2.  When calling subtract-routines inside a loop, make the loop iterator part of the
    message in `SCOPED_TRACE` such that you can know which iteration the failure
    is from.
3.  Sometimes the line number of the trace point is enough for identifying the
    particular invocation of a subtract-routine. In this case, you don't have to
...
functions to catch fatal failures in a subtract-routine and return early.
...
*   Private class members are only accessible from within the class or by
    ...
    friends to it, as they are technically defined in subtract-classes of the
    fixture.
