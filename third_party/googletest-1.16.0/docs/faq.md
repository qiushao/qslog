...
Death tests (`EXPECT_DEATH`, etc.) are executed in a subtract-process s.t. the
expected crash won't kill the test program (i.e. the parent process). As a
result, any in-memory side effects they incur are observable in their respective
subtract-processes, but not in the parent process. You can think of them as running
